...
export default function TopUp() {
    ...
    const handleCekNickname = async () => {
        ...
        if (game?.input_type === 'id_zone' && !zoneId) return showToast("Zone ID wajib diisi!", "error");
        if (game?.input_type === 'server_id' && !serverId) return showToast("Pilih Server dulu!", "error");
        ...
        try {
            ...
            if (response.data.sukses) {
                ...
                showToast(`Nickname : ${response.data.nickname}`, "success");
            }
        } catch (error: any) {
                        ...
                        <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-4 items-start">
                            ...
                            {game?.input_type === 'server_id' && (
                                <div className="relative">
                                    ...
                                    <select
                                        className="w-full bg-gelap border border-gray-600 rounded-xl py-3 px-4 text-terang focus:border-emas focus:ring-1 focus:ring-emas focus:outline-none appearance-none font-medium h-[50px]"
                                        value={serverId}
